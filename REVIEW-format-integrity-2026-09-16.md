...
**Format-integrity items 1–8 (2026-09-16, second round)** — implemented, deployed, services restarted

- **§2.4 SmartArt (item 1).** `app/core/smartart.py` is now the single enumerator, used by both extractor and injector, so the two walks cannot drift. Real Master-Nojima round trip: **41 SmartArt runs** out of 1265 (1224 + 41), all 41 marked, unique-donor guard clean. Guard: `tests/test_smartart_units.py` **17/17**.
- **§2.1 fonts (item 2).** The exported deck really renders `YuGothic-Regular`/`YuGothic-Bold` and all **1149** Japanese runs request the family — proven by rendering the deck, not by `fc-match`, because fontconfig substitutes silently. `/api/health` now reports `jp_font: {available: true, resolved: 'Yu Gothic,游ゴシック'}`.
- **Model IDs (item 3).** `validate_models()` + `is_missing_model_error()`, a per-model last-failure map that survives failover, and an opt-in startup probe (`VALIDATE_MODELS_ON_STARTUP`, background task, surfaced in `/api/health` as `model_errors`). First real probe: **5 problems** — `gemini-3-pro-preview` retired (→ `gemini-3.1-pro-preview`), and **all four OpenCode models dead** with `MissingSessionID`, because OpenCode Go has required the `x-opencode-session` header since 2026-09-05. After the header and registry fixes (`kimi-k2.5` gone → `kimi-k2.6`): **0 problems across 8 models**.
- **Golden-deck test (item 4).** `backend/tests/test_golden_deck.py`: real extractor → real job pipeline → real injector, stubbing **only** the network call. **13/13**, and it pins that two runs of one deck produce identical node text sequences.
- **Layer 2 vision QA (item 5).** `app/qa/render.py` (one PPTX→PDF→PNG path for every consumer), `app/qa/gemini_client.py`, `app/qa/vision_qa.py`. Opt-in, report-only. Live on the FADC job: **original deck** — 3/3 slides, 1 flagged, 2 issues (both high: text overflows the top boxes and collides with the arrows on slide 3); **translated deck** — 3/3 slides, 2 flagged, 5 issues, including an overlap on slide 3 that the original did not have (English needs more room than the Japanese it replaced). Vision judgement, not a pixel diff — it says where to look, not that injection broke it.
- **Layer 3 translation review (item 6).** `app/qa/translation_review.py`: deterministic leftover scan plus a terminology/register pass over the job's own units. Live on the FADC job, 40 units, **3 findings, 0 high** — `基盤` rendered both "Foundation" and "Infrastructure" in identical contexts; punctuation inconsistent across parallel bullets; and `データエンジニアリングで` turned into "In data engineering", shifting an instrumental into a locational sense. Deterministic leftover scan: 0.
- **API for both passes.** `POST /api/qa/vision`, `POST /api/qa/review`, `GET /api/qa/reports/{job_id}`; reports are persisted on the job (`qa_reports`) so a page reload does not re-spend the calls. Verified live: 200s, vision 11.7 s for 3 slides, review 200 with 0 errors.
- **Before/after preview (item 7).** `GET /api/source?job_id=` returns the deck as uploaded (verified: **14,580,700 B**, the upload's exact size); `/api/preview` takes `which=original|translated` and caches each side under its own key; `page.tsx` has a Before/After toggle. Verified through the frontend with a real session: original → **19 images**, fresh render 10.4 s; translated → **19 images** from its own cache in **0.05 s** (the two sides do not collide).
- **§2.8 (item 8).** The `[INJECTOR]` per-shape trace (hundreds of lines per deck) is gated behind `PPTX_VERBOSE=1`; the one-line-per-job summary and the error lines stay on. The `parts` shadowing and hardcoded model IDs were folded into items 3 and 6.

**Defect found while deploying (fixed).** The backend systemd *user* unit's PATH was the venv plus `~/.config`, so it could not see `/usr/bin` or `/usr/sbin`. Two user-visible lies followed: `/api/health` said **"fc-match unavailable — cannot verify"** and Layer 2 said **"cannot render slides: soffice, pdftoppm not installed"** — on a machine where all three tools exist and work from a shell. Fixed in code (`app/core/fonts.py` and `app/qa/render.py` now resolve absolute locations, and report missing only when they really are) *and* in the unit (`jpeigo-backend.service.d/path.conf`). Post-fix live check: `jp_font` reports installed, vision pass returns 200. The frontend unit was never affected — it inherits a PATH containing `/usr/bin`, which is why the existing preview route worked.

**Verification totals for this round:** jest **32/32** · `tsc --noEmit` clean · `npm run build` exit 0 · injector units pass · SmartArt **17/17** · golden deck **13/13** · model probe **0/8 problems** · `/api/source` 200 · QA vision 200 · QA review 200 · preview both sides 19 images each.

### Still open

- **Font decision (§2.1)** — `JP_FONT_FAMILY` defaults to `Yu Gothic`. The family is now installed here and the exported deck really renders `YuGothic-Regular/Bold`, but *which* family the output should use is still a product decision, not a code change.
- **QA reports have no UI.** Layer 2 and Layer 3 persist their reports on the job (`qa_reports`) and serve them from `GET /api/qa/reports/{job_id}`, but nothing renders them yet — today you read them from the endpoint or the CLI.
- **Layer 2 is per-slide vision calls**, so on a 39-slide deck it is a real spend; `max_slides` is the dial. Whether it should ever run automatically after export is still open — right now it is strictly opt-in.
